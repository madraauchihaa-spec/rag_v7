...
def validate_records(records):
    ...
    for r in records:
        if not r["sr_no"]:
            raise ValueError("Missing sr_no detected")

        if r["observation"] is None:
            r["observation"] = ""
...
def process_all_reports():
    ...
    for filename, config in REPORT_CONFIG.items():
        ...
        structured_records = validate_records(structured_records)

        output_filename = filename.replace(".md", "_structured.json")
        output_path = os.path.join(OUTPUT_FOLDER, output_filename)

        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(structured_records, f, indent=4, ensure_ascii=False)
